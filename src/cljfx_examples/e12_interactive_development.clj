...
(ns cljfx-examples.e12-interactive-development
...
;; because cljfx has no way to know if definition of some component function
;; is changed. But we can ask renderer to refresh itself by calling it without
;; any arguments.

(renderer)

;; Now small label got replaced with 2 slider. Problem is, there are no labels
...
;; What remains is propagating changes in slider values to our model, so let's add it.
...
;; Nice, now playing with sliders make the chart data change! And all this is
;; done in runtime! Looks like the problem was gravity too high, so as a final touch,
;; let's make it lower (both current and slider range)
